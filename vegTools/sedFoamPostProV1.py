...
def create_point_2Dcyl(X, Y, Z,precision = 10):
    """
    Extract positions of the bottom patch of the simulation
    and creates list of vertical extrusions for each of the cells
    from the bottom patch

    Inputs: 
         - X,Y,Z : 1D array of mesh-size corresponding to writeCellCenters of OF

    Outputs:
         - n2d : number of cells on the bottom 2D patch (which should be the same for each vertical z)
         - nz : number of cells in the vertical direction : PROBLÈME DANS CETTE DÉFINITION 
         - pbed : 1D array containing id. of the cells on the bottom patch
         - pointer : array of size (n2d,:) which contains, for each cell of the patch, the id of all
                     the cells located above (in ascending order of Zb) 
    """
    import numpy as np
    from tqdm import tqdm
    ...
    for i in tqdm(range(n2d)):
        ...
        indices = np.where(np.logical_and(np.round(X,precision) == np.round(X[pbed[i]],precision),
                                          np.round(Y,precision) == np.round(Y[pbed[i]],precision)))[0]
...
def uns2cart(array, nx, ny, nz, indcart, indorig):
    """
    Reorder the cells id. from 'unstructured' to 'structured' table of data.

    Inputs : 
         - array :  1D array containing fields to reorder (e.g : alpha.a , U.b ...)
         - nx : number of cells in the x-direction (which is not equal to the length of writeCellCentres of OF)
         - ny : number of cells in the y-direction (which is not equal to the length of writeCellCentres of OF)
         - nz : number of cells in the z-direction (which is not equal to the length of writeCellCentres of OF)
         - indcart : 2D array of id. for 'cartesian' ordering of size (nx,ny)
         - indorig : range of id. for the patch bottom : indorig = range(n2d)
...
class ThreedimSqrsimu(object):
    ...
    def createmesh(self, g0X, g2X, N0X, N1X, N2X, g0Y, g2Y, N0Y, N1Y, N2Y, gZ, gZ1, nzb, nz1, read_mesh, read_result):
        ...
        nvoid = nx*ny*nz - ncell #number of 'void' cells
